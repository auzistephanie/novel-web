...
import { getParentCategory, type CategoryColor } from "@/lib/genreCategories";
...
export default async function CategoriesPage() {
  ...
  const counts = new Map<
    string,
    { key: string; label: string; count: number; color: CategoryColor }
  >();
  for (const r of rows ?? []) {
  ...
  const genres = [...counts.values()].sort((a, b) => b.count - a.count);
    ...
    <main className="flex-1 max-w-4xl w-full mx-auto px-5 py-10">
      ...
      {genres.length === 0 ? (
        ...
        <div className="grid gap-4 grid-cols-[repeat(auto-fill,minmax(150px,1fr))]">
          {genres.map(({ key, count, label, color }) => (
            <Link
              key={key}
