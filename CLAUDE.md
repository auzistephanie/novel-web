...
`/login`（Supabase Auth）· `/`（首頁，拆「短篇故事」＋「每日連載」兩個 section，各自獨立 genre 篩選）· `/story/[id]`（全文＋結局）· `/my-endings`（我的結局本，登入後先見）· `/admin`（2026-07-15 加，故事列表＋刪除，淨 `auzistephanie@gmail.com` login 見到／`src/lib/admin.ts` 白名單）

⚠️ `/admin` 刪除故事：`novel_stories` DELETE RLS policy 淨限 admin email（2026-07-15 migration `novel_stories_admin_delete_and_cascade`），`novel_likes`／`novel_endings` 外鍵已改 `ON DELETE CASCADE`，刪故事自動連帶清走相關鍾意／結局，唔使 service-role key。
...
- `novel-story-generator` — 每日 12:30 同 17:30 各跑一次，每次都生成新一批 3 篇（唔 check 今日出過幾多，逐次疊加），固定比例 2 連載＋1 短篇，30 類別加權池（男頻 weight 1 / 女頻 weight 2；2026-07-11 已同步 daily-novel/docs/SYSTEMS.md 嘅 Phase 2 新增類別，兩邊 pool 一致）
...
- ⚠️ **推送本 repo（2026-07-15 更正，取代舊「普通 git CLI」寫法）**：呢個 repo 而家有自己嘅 `github_push.py`（GitHub API，PAT in `.env`，跟返全 repo 鐵律「永不用 git CLI」）——本機直接 `python3 github_push.py "<commit message>"`。⚠️ Cowork **雲端** sandbox 跑呢個 script 會撞 `403 GitHub access to this repository is not enabled for this session`（api.github.com 畀 sandbox 網絡政策擋咗，但單純 `git clone`/`git push` 走 github.com git protocol 唔受影響）——雲端遇到呢個 403，正確做法係用 `device_commit_files` 寫返 Mac，本機跑 `github_push.py` 完成 push，唔好喺雲端 sandbox 度改用 plain git CLI push 頂替。
- ⚠️ **Cowork sandbox 入面唔好直接喺呢個 mounted folder 度 `git commit`/`git fetch`**（2026-07-11 確認）：FUSE bridge 對 `.git` 內部寫入（temp object unlink、index）會報 `Operation not permitted`，令 `.git/index` 變到近乎空白，`git status` 會將成個 repo 顯示做 untracked。呢個唔係 stale lock 咁簡單可以刪咗就算，係 bridge 本身寫入 `.git` objects/index 唔穩定。**雲端要驗證改動時：喺 sandbox `/tmp` 度 fresh clone → 複製改動咗嘅檔案落去 → 喺嗰度跑 build 驗證**，唔好郁 mounted folder 嘅 `.git`；push 一律用上面嘅 `github_push.py` 流程（本機或已 allowlist 嘅雲端 repo）。
