...
## 題材頁大類分組（2026-07-11 加，唔改資料庫）

`novel_stories.genre` 係自由文字（scheduled task 生成時由 30 類池揀），隨時間累積會愈揀愈多細分類，`/categories` 頁如果直接按 `genre` 分組會愈來愈碎（曾經一度有 23 個細類，大部分得 1 篇故事）。

修法：`src/lib/genreCategories.ts` 定義 7 個大類（`PARENT_CATEGORIES`），每個大類收埋幾個細類 `genre` 字串；`/categories` 頁改用 `getParentCategory(genre)` 分組計數，連結變 `/browse?parent=<key>`；`/browse` 頁對應加 `parent` 參數，用 `.in("genre", parent.genres)` 撈晒個大類底下所有細類故事；原本 `?genre=<細類>` 直連寫法（首頁「熱門題材」用緊）保持向下兼容，冇改。

7 大類：總裁甜寵、復仇虐戀、馬甲身份、翻身逆襲、古言宮廷、系統腦洞、都市日常。判斷準則：跟「核心情緒賣點」分，唔跟「敘事機制」（例如「重生」淨係手法，唔應該獨立成大類，歸入「翻身逆襲」）。

⚠️ 呢個 mapping 表要人手維護——未來如果 30 類池又加新類別，記得同步喺 `genreCategories.ts` 加返對應大類，否則新類別會跌落 fallback「其他」桶。
