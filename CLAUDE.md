...
- ⚠️ **Cowork sandbox 入面唔好直接喺呢個 mounted folder 度 `git commit`/`git fetch`**（2026-07-11 確認）：FUSE bridge 對 `.git` 內部寫入（temp object unlink、index）會報 `Operation not permitted`，令 `.git/index` 變到近乎空白，`git status` 會將成個 repo 顯示做 untracked。呢個唔係 stale lock 咁簡單可以刪咗就算，係 bridge 本身寫入 `.git` objects/index 唔穩定。**正確做法：喺 sandbox `/tmp` 度 fresh clone → 複製改動咗嘅檔案落去 → 喺嗰度 commit + push**，唔好郁 mounted folder 嘅 `.git`（本機你自己電腦開 Terminal 跑就冇呢個問題，一樣可以直接 `git commit`）。
