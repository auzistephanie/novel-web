...
- 邏輯全部喺 `route.ts`：揀「骨架」（identity_reveal/contract_marriage/power_clash/mutual_healing，4:3:2:3 權重）→ 獨立 slot 池組情節 → `gen_meta` 記低落 DB 俾下次排除近期組合；DeepSeek 生成＋code validate＋self-check，唔合格 retry 3 次；heartbeat 喺 route 尾段 upsert。`STYLE_2026` 2026-08-18 由清單式規則改做5條創作原則，緣由同四個骨架/揭露機制詳情 → daily-novel `CHANGELOG.md` 2026-08-13／2026-08-18
